...
class Puyo:
    ...
    Puyo_Type_list = range(6) #4색으로 변경하면 오류남. 왜?
    # BLANK = 0
    # RED = 1
    # BLUE = 2
    # GREEN = 3
    ...
    sub_puyo_muki = 2  # 0:right, 1:down, 2:left, 3:up
    ...
    def __init__(self):
        ...
        self.DISP_FIELD_SIZE = (
            self.PUYO_RADIUS * 2.17 * self.COL_NUM,
            self.PUYO_RADIUS * 2.1 * (self.ROW_NUM - 1), # 필드 크기
        )
        ...
        self.holded_puyo_pos = copy.copy(self.holded_puyo_pos) # 나중에
    ...
    def draw_frame(self):
        pygame.draw.rect(
            ...
            (
                ...
                self.DISP_FIELD_SIZE[0] + self.BORDER ,
                self.DISP_FIELD_SIZE[1] + self.BORDER ,
            ),
            self.BORDER,
        )
        pygame.draw.rect(
        ...
        pygame.draw.rect(
            ...
            self.C_BROWN,
            (
                self.DISP_FIELD_TOP_LEFT[0] * 1.4 - self.DISP_FIELD_SIZE[0],
                self.DISP_FIELD_TOP_LEFT[1] - self.BORDER,
                self.PUYO_RADIUS * 6 + self.BORDER,
                self.PUYO_RADIUS * 8 + self.BORDER,
            ),
    ...
    def hold_puyo(self): #홀드
        # 이미 홀드된 뿌요가 존재하는가?
        # 아니면 없는가?
        if self.hold_puyo is None :
            self.holded_puyo_pos[0] = self.cur_puyo_pos[0], self.cur_sub_puyo_pos[0]
        else:
            self.holded_puyo_pos[0], ((self.cur_puyo_pos[0], self.cur_sub_puyo_pos[0])) = ((self.cur_puyo_pos[0], self.cur_sub_puyo_pos[0])), self.holded_puyo_pos[0]
        return self.cur_puyo_pos[0], self.cur_sub_puyo_pos[0], self.holded_puyo_pos[0]
    
   
    
    def loop(self):
        
        self.hold_used = False #홀드
        self.holded_puyo = None
        ...
        for event in pygame.event.get(KEYDOWN):
            ...
            elif event.key == K_d: #왜 하면 튕기지? 확인1
                if  not self.hold_used:
                    self.cur_puyo_pos[0], self.cur_sub_puyo_pos[0] = self.hold_puyo
                    self.hold_used = True
        ...
        if self.game_state == GameState.FALLING:
            ...
            if (
                ...
                if self.sub_puyo_muki == 0 or self.sub_puyo_muki == 2:
                    # 뿌요 밑에 뿌요나 바닥이 있을때 メインの下に何かある（ぷよか壁）もしくは サブの下に何かある（ぷよか壁） 場合
                    if (
                        self.puyo_field[self.cur_puyo_pos[0]][self.cur_puyo_pos[1] + 1]
                        != self.Puyo_Type_list[0]
                        ...
                        # 착지한 상태로 만듬(着地)
                        self.game_state = GameState.ON_PUYO
                    # 두뿌요 밑에 아무것도없을때(なんもないとき）
                    else:
                        # 두뿌요 다 하나씩 아래로 내림 サブとメインを1つ下にずらす
                        main_color = self.puyo_field[self.cur_puyo_pos[0]][
                            self.cur_puyo_pos[1]
                        ]
                        sub_color = self.puyo_field[self.cur_sub_puyo_pos[0]][
        ...
        if self.game_state == GameState.GAMEOVER: #gameover
            self.screen.blit(self.text1, (400, 300))

        self.screen.blit(self.text2, (250, 400))  # rensa
        self.text3 = self.font2.render(str(self.score_rensa_num), True, (255, 255, 255))
        self.screen.blit(self.text3, (235, 400)) # rensa num
